...
{\ttfamily -no-gen-CBOR} & {\small Do not generate the CBOR (RFC~8949) support code.}\\
...
RFCs~8742, 8610, 9052, and 9581. CBOR support is generated by default, the same
as BER, XER, OER, PER, and JER. To omit CBOR support from the generated output,
use the \cmd{-no-gen-CBOR} flag:

\begin{bash}
asn1c -no-gen-CBOR %\textbf{rectangle.asn}%
\end{bash}

The compile-time guard \code{ASN\_DISABLE\_CBOR\_SUPPORT} controls whether CBOR
symbols are compiled into the runtime. When \cmd{-no-gen-CBOR} is used,
