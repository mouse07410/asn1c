 ...
 */
/*
 * SEQUENCE OF / SET OF CBOR codec.
 * SEQUENCE OF encodes as a CBOR array (major type 4).
 */
...
asn_enc_rval_t
SEQUENCE_OF_encode_cbor(const asn_TYPE_descriptor_t *td,
    ...
    /* Write CBOR array header with element count */
...
SEQUENCE_OF_decode_cbor(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    list = _A_SEQUENCE_FROM_VOID(*struct_ptr);
    ...
    if(arr_count == (uint64_t)-1) ASN__DECODE_FAILED;  /* No indefinite */
