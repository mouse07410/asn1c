...
static void
check_xer_output(const char *xer) {
	assert_contains(xer, "<payload>SGVsbG8=</payload>");
	assert_contains(xer, "<named>V29ybGQ=</named>");
	assert_contains(xer, "<prefixed>UHJlZml4</prefixed>");
	assert_contains(xer, "<embedded>TmVzdA==</embedded>");
	assert_contains(xer, "<setEmbedded>U2V0</setEmbedded>");
	assert_contains(xer, "<chosen>Q2hvaWNl</chosen>");
	assert_contains(xer, "T25l");
	assert_contains(xer, "VHdv");
	assert_contains(xer, "UmVk");
	assert_contains(xer, "Qmx1ZQ==");

	assert_not_contains(xer, "48656C6C6F");
	assert_not_contains(xer, "576F726C64");
	assert_not_contains(xer, "507265666978");
	assert_not_contains(xer, "4E657374");
	assert_not_contains(xer, "536574");
	assert_not_contains(xer, "43686F696365");
	assert_not_contains(xer, "4F6E65");
	assert_not_contains(xer, "54776F");
	assert_not_contains(xer, "526564");
	assert_not_contains(xer, "426C7565");
}
...
encode_decode_message(void) {
	...
	xer_len = 0;
	er = xer_encode(&asn_DEF_Message, &src, XER_F_CANONICAL,
	                buf_writer, 0);
	assert(er.encoded > 0);
	assert(xer_len > 0);
	check_xer_output(xer_buf);

	dr = xer_decode(0, &asn_DEF_Message, (void **)&decoded,
	                xer_buf, xer_len);
	assert(dr.code == RC_OK);
	check_decoded_message(decoded);

	xer_len = 0;
	er = xer_encode(&asn_DEF_Message, decoded, XER_F_CANONICAL,
	                buf_writer, 0);
	assert(er.encoded > 0);
	check_xer_output(xer_buf);
